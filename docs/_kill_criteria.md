...
| K-W5-3 | Owner-initiated rug-pull rate across all causes (planned exit + sale-escape + default + non-renewal + adversarial), trailing 12-month rolling | **> 5 per 100 domain-years → execute decision menu (see W5 §9.11): suspend new sale-escapable supply / require registrar escrow for new Bonded Premium / raise exit fees / reduce Standard cap below 30% / reprice migration reserve into renter pricing.** Steady-state metric. | EXECUTE DECISION MENU | Domain lifecycle ledger | W5 / W7 |
| K-W5-3a | First owner-initiated rug-pull (any cause) within first 12 months of marketplace operation | **Any occurrence → FREEZE new Standard sale-escapable onboarding pending root-cause review; reduce Standard cap from 30% → 20%** | FREEZE / TIGHTEN | Domain lifecycle ledger; ops alerting | W5 / W7 |
| K-W5-3b | Any Bonded Premium owner attempts a Change of Control (per W5 §9.3 anti-avoidance triggers) | **Any occurrence → SUSPEND all Bonded Premium product claims in marketing; trigger immediate registrar-control architecture review under §9.1; assert §9.1 controls + §9.3 liquidated damages** | SUSPEND / ARCHITECTURE REVIEW | Lease compliance + WHOIS monitoring | W5 / W7 |
| K-W5-3c | Single rug-pull event affecting > 25 renters | **Any occurrence → IMMEDIATE W7 reforecast; pause new domain onboarding until reforecast completes** | IMMEDIATE REFORECAST | Migration ops ledger | W5 / W7 |
| K-W5-4 | Regulatory complaint rate (CAN-SPAM / state-AG / trademark-takedown — US scope per §9.10) per 1,000 renter-months trailing 12-month rolling | **> 1 → escalate to legal review; > 3 → architecture pause / launch-blocking re-open** | ESCALATE / PAUSE | Legal/ops complaint ledger | W5 / W6 |
| K-W5-5 | Indemnification reserve cumulative draw-down against $100k annual aggregate cap (per W5 §9.4 raised cap; claims paid + outstanding reserved) | **> 50% in any rolling 12-month window → tighten AUP enforcement / raise insurance limit / re-examine cap structure; > 80% → architecture review** | TIGHTEN / RAISE / REVIEW | Indemnification ledger; insurance broker statement | W5 / W7 |
